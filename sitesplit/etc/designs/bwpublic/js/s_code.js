...
/***************************************************/
/**/
/*    s_code.js file for Barclays Wealth Management */
/*    Created on 18th December 2012 by Tom Marianczak of Adversitement BV */
/*	  v1.5 signifies after QA on UAT.	*/
/*	  v1.6 Has updated H code H26	*/
/*    v1.7 has new domains + 404 reporting + Trading Hub + IBF */
/*    v1.8 has SmartInvestor and InternationalWealth  */

var last_modified_on = "2014-07-09"; //YYYY-MM-DD
var last_edited_by = "GF"; //initials only please
var version_no = "1.8.6";
/**/
/***************************************************/
var adv = new Object(); //setup the adversitment object
var s_account = "barcapwealthinternetdev";  //default to dev RSID
var num_periods = 2;  //default to .com period count

//autodetect RSID
if (document.domain.match(/wealth.barclays\.com/g) || document.domain.match(/apply.barclays\.co\.uk/g) || document.domain.match(/www-gem\.barclaystradinghub\.co\.uk/g) || document.domain.match(/www-pi\.barclaystradinghub\.co\.uk/g) || document.domain.match(/www\.barclaystradinghub\.co\.uk/g)) {
    s_account = "barcapwealthinternetprod";  //default to dev RSID
}

var s = s_gi(s_account);  //now setup the s object
...
s.linkInternalFilters = "javascript:,barcapint.com,barclayswealth.com,31.222.138.48,sapient.com,localhost,mailto:,wealth.barclays.com,apply.barclays.co.uk,barclaystradinghub.co.uk";
s.linkLeaveQueryString = false;
s.linkTrackVars = "None";
s.linkTrackEvents = "None";
s.cookieDomainPeriods = num_periods;
...
if(!s.c_r("s_ss")){
	//set the time start session cookie
	var ts = new Date;
	ts = ts.getTime();
	s.c_w("s_ss", ts, 0);
}
...
function s_doPlugins(s) {
    ...
    if(!s.isCustomLinkTrack){
    
        //initialise the object of meta values
        s.metavalues = s.getMetaValues();
        s.urlparams = s.getURLParameters();
        
        if(s.metavalues['events'] && s.metavalues['events'] != ""){
            s.events = s.metavalues['events'];
        }
        
		//IBF: grab querystring parameters from page meta
		if(s.metavalues['url-params'] && s.metavalues['url-params'] !== '') {
			var properties = s.metavalues['url-params'].split(', ');
			s.urlparams = {};
			properties.forEach(function(property) {
				var tup = property.split(':');
				s.urlparams[tup[0]] = tup[1];
			});
		}
		
        //only do this outside of the a custom link traking    
    
        var jsondecoded = jsondecoded || {}; //just defined some variables for use

        if (s.metavalues['adv_server']) {
            s.server = s.metavalues['adv_server'].toLowerCase();  //sub section
        }
        
        if (s.urlparams.cid) {
            //set campaign if the external campaign id is set (cid)
            s.campaign = s.urlparams.cid;
        }else if(s.urlparams['WT.mc_id']){
			s.campaign = s.urlparams['WT.mc_id'];
		}
        if (s.urlparams.icid) {
            //set eVar8 if the internal campaign id is set (cid)
            s.eVar8 = s.urlparams.icid;
        }
    	
    	if (s.metavalues['adv-site-section']) {
    		s.channel = s.metavalues['adv-site-section'].toLowerCase();  //section	
    	} else if(s.metavalues['adv-content-name']) {
    		s.channel = s.metavalues['adv-content-name'].toLowerCase();  //content name
    	} else {
            s.channel = "not specified";
        }
        
        if (s.metavalues['adv-subsection']) {
            s.prop1 = s.metavalues['adv-subsection'].toLowerCase();  //sub section
        } else {
            s.prop1 = "not specified";
        }
        s.eVar1 = s.eVar1 || "D=c1";
    	if (s.metavalues['adv-content-type']) {
    		s.prop2 = s.metavalues['adv-content-type'].toLowerCase();  //content type	
    	} else {
    		s.prop2 = "not specified";
    	}
        
        s.eVar2 = s.eVar2 || "D=c2";
        if (s.metavalues['adv-content-name']) {
            s.prop3 = s.metavalues['adv-content-name'].toLowerCase();  //content name    
        } else {
            s.prop3 = "not specified";
        }
        
        s.eVar3 = s.eVar3 || "D=c3";
        
        var pnss = "";  //page name sub section
        
        if(s.prop1 !== "not specified"){
            pnss = ":"+s.prop1;    
        }
    	
    	var chss = ""; //channel sub section
    	if(s.channel !== "not specified" && (s.channel !== "home" && s.prop3 !== "home")){
            chss = ":"+s.channel;    
        }

        //create pagename and hier1 as [site-prefix]:[section]:[sub-section}:[content-name]
        adv.site_prefix = "bwm"; //default prefix to "bwm"
		if(document.domain.match(/barclaystradinghub\.co\.uk/g)) {
			adv.site_prefix = "bth";	
		} else if(document.domain.match(/apply\.barclays\.co\.uk/g)) {
			adv.site_prefix = "ibf";
			// Setting form name variable
			if (s.metavalues['adv-form-name'] && s.metavalues['adv-form-name'] !== "") {
				s.eVar10 = s.metavalues['adv-form-name'];
			} else {
				s.eVar10 = "IBF - Normal flow";
			}
		} else if(document.URL.match('smartinvestor') || document.URL.match('internationalwealth')){
			// Smart Investor or InternationalWealth
			adv.site_prefix = "ocp";
		}
		if(s.prop3 === "404") {
			s.pageType="errorPage";
		} else {
			s.pageName = s.hier1 = adv.site_prefix + chss + pnss + ":" + s.prop3;
		}
		
		//hack the vars for search results page.
		if(s.urlparams.q){
			s.pageName = s.hier1 = "bwm:others:search-results:search-results";
			s.channel = "others";
			s.prop1 = "search-results";
			s.prop3 = "search-results";
		}
		
		if(s.prop3 == "search-results"){
			s.prop2 = s.prop3;
		}

        s.prop4 = document.location.host + document.location.pathname; //get the current url with no params or hash anchors
        s.eVar4 = s.eVar4 || "D=c4";

        s.eVar6 = s.eVar6 || "D=pageName";
        s.eVar7 = s.eVar7 || "D=ch";

        //setup the versioning
        s.prop7 = version_no + ":" + last_modified_on + ":" + last_edited_by;

        if (s.metavalues['adv-int-search-results']) {
            s.eVar16 = s.metavalues['adv-int-search-results'];  //internal search term
        }
		
		if(typeof(adv_int_search_results) != "undefined"){
			s.eVar16 = adv_int_search_results;
		}

        s.eVar21 = "D=s_vi";    //visitor id sorted in sc backend

        if (s.metavalues['adv-account-id']) {
            s.eVar22 = s.metavalues['adv-account-id'];  //account id
        }

        //both variables in GMT
        s.eVar24 = s.getTimeParting("d", "0"); //day of week
        s.eVar25 = s.getTimeParting("h", "0"); //hour of day

        //new repeat visitor
        s.eVar27 = s.getNewRepeat(60, 's_gnr');

        if (s.metavalues['adv-form-start']) {
            s.events = s.apl(s.events, 'event3', ',', 1);
            jsondecoded = eval("(function(){return " + s.ensurevalidjson(s.metavalues['adv-form-start']) + ";})()");
            s.eVar10 = jsondecoded['form-1'];  //account id
        }

        if (s.metavalues['adv-form-complete']) {
            s.events = s.apl(s.events, 'event4', ',', 1);
            jsondecoded = eval("(function(){return " + s.ensurevalidjson(s.metavalues['adv-form-complete']) + ";})()");
            s.eVar10 = jsondecoded['form-1'];  //account id
			s.eVar11 = s.timeBeforeApplication();
        }

        //reset a cookie that helps with firing a custom link tracking only if the page has changed.
        if (s.c_r('s_evt15') && s.c_r('s_evt15') !== s.pageName) {
            s.c_w('s_evt15', s.pageName, 0);
        }
		
		if(window.document.location.pathname.match(/contactform\.html/i)){
			s.channel = "contact-us";
			s.prop2 = "online-form";
			s.prop3 = "online-contact-us";
			s.eVar10 = "bwm:online-contact";
            s.events = s.apl(s.events, 'event3', ',', 1);
			
			s.v31p = s.v31p || [];
			if(s.urlparams.existingClient){
				s.v31p.push(s.urlparams.existingClient);
			}
			if(s.urlparams.selectedCountry){
				s.v31p.push(s.urlparams.selectedCountry);
			}
			if(s.urlparams.selectedService){
				s.v31p.push(s.urlparams.selectedService);
			}
			s.eVar31 = s.v31p.join('|');
			
			s.hier1 = s.pageName;
		}
		if(window.document.location.pathname.match(/onlineformsuccess\.html/i)){
			s.channel = "contact-us";
			s.prop2 = "online-form";
			s.prop3 = "online-contact-us";
			s.eVar10 = "bwm:online-contact";
			...
			//Contact us confirmation page tracking
			if (s.metavalues['adv-contact-country'] && s.metavalues['adv-contact-department']) {
				s.eVar32 = s.metavalues['adv-contact-country'].toLowerCase();  //country selected in drop-down menu	
				s.eVar33 = s.metavalues['adv-contact-department'].toLowerCase();  //department selected in drop-down menu	
			} 
    ...
    }//end if custom link track
    s.isCustomLinkTrack = false;
}
...
s.getMetaValues = function () {
    //this function returns all meta values as an object
    var metavalues = {}, my_arr = document.getElementsByTagName("meta"), counter = 0;

    for (counter = 0; counter < my_arr.length; (counter++)) {
        metavalues[my_arr[counter].getAttribute('name')] = my_arr[counter].content;
    }
    return metavalues;
};

s.getURLParameters =  function () {
...
s.ensurevalidjson = function(jsonstring){
    return jsonstring.replace(/"/g, '"');
}

s.timeBeforeApplication = function(){
	s.act = new Date;
	s.act = s.act.getTime();
	s.ttt = Math.round((s.act - s.c_r("s_ss"))/1000);  //seonds
	if(s.ttt > 60){
		s.ttt = Math.round(s.ttt/60); //minutes
		if(s.ttt > 60 ){
			s.ttt = Math.round(s.ttt/60); //hours
			if(s.ttt > 24 ){
				s.ttt = Math.round(s.ttt/24); //days
				s.c_w("s_ss", "");
				return s.ttt+" days";
			}else{
				s.c_w("s_ss", "");
			...
			s.c_w("s_ss", "");
			return s.ttt+" minutes";
		}
	}else{
		s.c_w("s_ss", "");
		return s.ttt+" seconds";
	}
}
...
/***********Adversitement Functions *********/
function navigate(){
    window.setTimeout(function(){window.location = adv.thelink},500);
}
//utility function to track various events using a custom link tracking.
adv.trackEvent = function (eventtype, contenttype, contentname, extservicename, formname, intsearchphrase, intsearchresults, intsearchtype, interactiontype, sharetype, ctatype, goalstmtused, rejectedcookies, thelink) {
 	
    s.isCustomLinkTrack = true;
    s.newlocation = false;
    
    //default the params if necessary
    var eventstotrack = [], varstotrack = [], linktracktype = 'o', linkname;

    varstotrack.push('events');

    if (eventtype.toLowerCase() === 'cookies') {
        if (rejectedcookies === null || rejectedcookies == "") {
            eventstotrack.push('event1');
            linkname = "cookies-accepted";
        } else {
            eventstotrack.push('event19');
            s.prop6 = rejectedcookies;
            varstotrack.push('prop6');
            linkname = "cookies-rejected";
        }

    } else if (eventtype.toLowerCase() === 'external') {
        eventstotrack.push('event2');
        s.eVar9 = extservicename;
        varstotrack.push('eVar9');
		linktracktype = "e";
        linkname = "external-service-clickout:" + s.eVar9;

    } else if (eventtype.toLowerCase() === 'search') {
        
        s.eVar17 = intsearchtype;
        varstotrack.push('eVar17');
		
		if(intsearchtype.match('refinement')){
			eventstotrack.push('event13');
		} else if (goalstmtused === null || goalstmtused == "" && !intsearchtype.match('refinement')) {
            eventstotrack.push('event12');
			s.eVar15 = intsearchphrase;
			varstotrack.push('eVar15');
        } else if (goalstmtused != null || goalstmtused != "" && !intsearchtype.match('refinement')) {
            eventstotrack.push('event20');
            s.eVar26 = goalstmtused;
            varstotrack.push('eVar26');
        }

        linkname = "internal-search:" + s.eVar17;

    } else if (eventtype.toLowerCase() === 'content') {
        
        s.eVar2 = contenttype;
        s.eVar3 = contentname;
        
        s.eVar18 = interactiontype;
        varstotrack.push('eVar2', 'eVar3', 'eVar18');

        switch (interactiontype.toLowerCase()) {
        case 'add-to-collection':
            eventstotrack.push('event14');
            break;
        default:
            eventstotrack.push('event16');
        }
        linkname = "content-interaction:" + s.eVar18;

    } else if (eventtype.toLowerCase() === 'collection') {
        eventstotrack.push('event15');
        s.eVar2 = 'view-my-collections';
        s.eVar3 = 'view-my-collections';
            
        varstotrack.push('eVar2', 'eVar3');
        linkname = "view-my-collection";

    } else if (eventtype.toLowerCase() === 'share' && sharetype !== null) {
        eventstotrack.push('event17');
        s.eVar2 = contenttype;
        s.eVar3 = contentname;
        s.eVar19 = sharetype;
        varstotrack.push('eVar2', 'eVar3', 'eVar19');
        linkname = "content-share:" + s.eVar19;

    } else if (eventtype.toLowerCase() === 'contact-us') {
        eventstotrack.push('event23');
		s.eVar10 = "bwm:online-contact";
    	if(contenttype == 'country') {
    		s.eVar32 = contentname;  //country selected in drop-down menu	
    	} else 	if (contenttype == 'department') {
    		s.eVar33 = contentname;  //department selected in drop-down menu
		}			
        varstotrack.push('eVar10', 'eVar32', 'eVar33');
        linkname = "contact-us:" + contenttype + ":" + contentname;

    } else if (eventtype.toLowerCase() === 'cta-click') {
        if( ctatype.match(/^1/i) ){
            eventstotrack.push('event21');  
        } else if( ctatype.match(/^2/i) ){
            eventstotrack.push('event22');    
        } else if( ctatype.match(/^3/i) ){
            eventstotrack.push('event23');    
        } else if( ctatype.match(/^4/i) ){
            eventstotrack.push('event24');    
        }else if( ctatype.match(/^5/i) || ctatype.match(/^9/i) ){
            eventstotrack.push('event29');    
        }else if( ctatype.match(/^6/i) ){
            eventstotrack.push('event26');    
        } else if( ctatype.match(/^7/i) ){
            eventstotrack.push('event27');    
        } else if( ctatype.match(/^8/i) ){
            eventstotrack.push('event28');    
        }
        
        s.eVar20 = ctatype;
        varstotrack.push('eVar20');
        linkname = "call-to-action:click";
        
    } else if (eventtype.toLowerCase() === 'cta-submit') {
       if( ctatype.match(/^1/i) ){
            eventstotrack.push('event31');  
        } else if( ctatype.match(/^2/i) ){
            eventstotrack.push('event32');    
        } else if( ctatype.match(/^3/i) ){
            eventstotrack.push('event33');    
        } else if( ctatype.match(/^4/i) ){
            eventstotrack.push('event34');    
        }
        
        s.eVar20 = ctatype;
        varstotrack.push('eVar20');
        linkname = "call-to-action:submit";
        
    }

    //we add the PageName in eVar6 in all link
    varstotrack.push('eVar6');

    //now we need to create a link name for the tracklink Function
    linkname = adv.site_prefix + ":" + linkname;
    ...
    if(thelink){
        adv.thelink = thelink.href;
        s.tl(linkname, linktracktype, linkname, null, navigate);
    }else{
        s.tl(linkname, linktracktype, linkname);
    }
    
    //clear down the link tracking parameters parameter
    s.linkTrackVars = 'None';
    s.linkTrackEvents = 'None';
...
s.visitorNamespace = "barclayscapital";
s.trackingServer = "barclayscapital.112.2o7.net";
